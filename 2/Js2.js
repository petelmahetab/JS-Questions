...
const arrayURLImges = [
    ...
    "https://picsum.photos/200300?random=5"
];
...
async function handlingPromise(){
    try{
        let loadedImg=await Promise.all(arrayURLImges.map(helperForImageGeneration));
        ...
        new Error(`Failed for reason ${e.message}`)
    }
}
...
function helperForImageGeneration(src){
   return new Promise((res,rej)=>{
        ...
        img.onerror=function (){
            rej(new Error('Failed to load images.'))
        }
        img.src=src;
    })
}
